...
for (i in pos_adj_walls) {
  if (file$'BuildingSurface:Detailed'[[i]]$'outside_boundary_condition' == 'Surface') {
    if (any(zone_surfs ==
      ...
      adiab_surfs = c(adiab_surfs, names(file$'BuildingSurface:Detailed')[i])
    }
  }
}
...
for (i in pos_one_fen) {
  ...
  fen_surfs[[i]] =
    double_window(fen_surfs[[i]], stringr::str_sub(names(fen_surfs)[i], -2, -2), 1)
  fen_surfs[[length(fen_surfs)]] =
    double_window(fen_surfs[[length(fen_surfs)]],
                  stringr::str_sub(names(fen_surfs)[length(fen_surfs)], -2, -2), 2)
}
...
for (i in pos_one_afn) {
  ...
  afn_surfs[[i]]$surface_name = stringr::str_sub(names(afn_surfs)[i], 5)
  afn_surfs[[length(afn_surfs)]]$surface_name =
    stringr::str_sub(names(afn_surfs)[length(afn_surfs)], 5)
}
