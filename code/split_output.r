...
DefZones = function(input_path, habs, nstrs) {
  ...
  if (count == 0) {
    zones = paste0('f', 1:nstrs) %>% expand.grid(habs, rooms) %>%
...
ApplySplOut = function(typo, input_dir, nstrs, output_dir, cores_left) {
  if (typo == 'l') {
  ...
  input_paths = dir(input_dir, paste0('_', typo, '_.*csv$'), full.names = TRUE)
...
SplitOutput = function(input_path, zones, output_dir, cores_left) {
  ...
  # zones: names of the zones to be splitted
  # output_dir: directory where the splitted files will be saved
  # cores_left: cores left out of the parallelism
  # load data
