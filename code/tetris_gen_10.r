    ...
    for (bound in bounds) { # for all boundaries
      ...
      if (bound[[1]][2] == 'c') { # core
        ...
        for (n in 1:2) { # run for interior and exterior hives
          if (bound[[n]][3] != 'outdoors') { # boundary is a room ('liv' or 'dorm')
            # 'x' origin vertex
      ...
      if (bound[[1]][2] == 'c') { # core
        for (hive in hives) { # run for hives boundary conditions
          ...
          seed$'BuildingSurface:Detailed'[[paste0(bound[[1]][1], '_wall_', hive[2])]]$
            'outside_boundary_condition' = ifelse(hive[3] == 'outdoors', 'Outdoors', 'Surface')
          if (hive[3] != 'outdoors') { # boundary is a room ('liv' or 'dorm')
        ...
        if (bound[[1]][3] == 'adiabatic') {
          for (a in 1:2) {
            for (n in 1:2) {
              # construction name
              seed$'BuildingSurface:Detailed'[[paste0(bound[[n]][1], '_wall_',
      ...
      if (bound[[1]][2] == 'c') { # core
        ...
        if (is_room(bound[[1]][3])) { # boundary is a room ('liv' or 'dorm')
          ...
          if (room == 'liv') { # boundary is a room ('liv'
            seed$'AirflowNetwork:MultiZone:Surface'[[paste0('afn_', bound[[2]][1],
                                                            '_window_', bound[[1]][2])]]$
              'venting_availability_schedule_name' = 'sch_afn_liv'
          } else { # boundary is a room ('liv'
      ...
      if (bound[[1]][2] == 'c') { # core
        ...
        for (n in 1:2) { # run for interior and exterior hives
          ...
          } else if (bound[[n]][3] == 'adiabatic') {
            # remove people internal loads
            seed$'People'[[paste0('people_', bound[[1]][1])]] = NULL
            # remove lights internal loads
            seed$'Lights'[[paste0('lights_', bound[[1]][1])]] = NULL
            # remove electric equipament internal loads
            seed$'ElectricEquipment'[[paste0('equip_', bound[[1]][1])]] = NULL
            if (room == 'dorm') {
              # number of occupants
              seed$'People'[[paste0('people_', bound[[2]][1])]]$
                'number_of_people' = 2
              # activity level schedule
              seed$'People'[[paste0('people_', bound[[2]][1])]]$
                'activity_level_schedule_name' = 'sch_activ_dorm'
              ...
              seed$'People'[[paste0('people_', bound[[2]][1])]]$
                'number_of_people_schedule_name' = 'sch_occup_dorm'
              # lights schedule
              seed$'Lights'[[paste0('lights_', bound[[2]][1])]]$
                'schedule_name' = 'sch_ilum_dorm'
              # equipment schedule
              ...
              seed$'Lights'[[paste0('lights_', bound[[2]][1])]]$
                'schedule_name' = 'sch_ilum_liv'
              # equipment schedule
              seed$'ElectricEquipment'[[paste0('equip_', bound[[2]][1])]]$
                'schedule_name' = 'sch_equip_liv'
            }
          } else { # boundary is outdoors
            # remove people internal loads
