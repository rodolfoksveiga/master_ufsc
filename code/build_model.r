...
RnmStorey = function(tags, storey, index) {
  # tags: vector with names of surfaces, zones or wathever....
  # storey: one specific storey generated by LabelStoreys function
  # single familiy - single / multi family - bot, mid or top
  # index: storey index
  # usually the first floor is 1, second is 2 and so on..
  if (storey != 'single') {
...
AddFields = function(object, fields, values) {
  # object: list containing the object
  # fields: vector of strings corresponding to the fields
  # values: vector of strings corresponding to the fields' values
  tags = names(object)
...
AddIntLoad = function(tag, fill, type) {
  # tag: zone or zone list name
  # fill: the list to fill
  # type: equip, light or people
...
AddZoneList = function(room, tags, fill) {
  ...
  AddZoneToList = function(x, y) append(y, list('zone_name' = x))
  list$zones = lapply(tags, AddZoneToList, list$zones)
  return(list)
}
...
ApplyIntLoads = function(type, fill, tags) {
  # tags: names of zones or zone lists
  if (type == 'equip') {
...
# application ####
grid = expand.grid('shell' = c('ref', 'tm', 'tv', 'sf'),
