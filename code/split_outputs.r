...
DefZones = function(input_path, dwels, n_strs) {
  case =  str_extract(input_path, '(?<=(ref|tm|tv|sf)).*')
  count = str_count(case, '_')
  rooms = c('liv', 'dorm_1', 'dorm_2')
  if (count == 0) {
    zones = paste0('f', 1:n_strs) %>%
      expand.grid(dwels, rooms) %>%
...
ApplySplOut = function(input_dir, output_dir, n_strs, dwels, cores_left = 1) {
  input_paths = dir(input_dir, '\\.csv$', full.names = T)
  zones = lapply(input_paths, DefZones, dwels, n_strs)
...
SplitOutput = function(input_path, zones, output_dir, cores_left) {
  ...
  prefix = str_remove(basename(input_path), '.csv')
...
# application ####
ApplySplOut('~/Desktop/test/output/', '~/Desktop/test/output/split/', 5,
            c('csw', 'msw', 'mse', 'cse', 'cne', 'mne', 'mnw', 'cnw'))
